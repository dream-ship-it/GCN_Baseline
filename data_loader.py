...
def load_data(dataset_name):
    ...
    if dataset_name in ['cora', 'citeseer', 'pubmed']:
        ...
        dataset = NodePropPredDataset(name="ogbn-cluster", root=path)
        graph, labels = dataset[0]  # Get the first graph object and the labels.
        edge_index = torch.from_numpy(graph['edge_index'])
        y = torch.from_numpy(labels.squeeze())
        x = torch.from_numpy(graph['node_feat'])
        split_idx = dataset.get_idx_split()
        train_mask = torch.from_numpy(split_idx["train"])
        val_mask = torch.from_numpy(split_idx["valid"])
    ...
    if dataset_name in ['computers', 'photo', 'cs', 'physics', 'wikics']:
        ...
        train_mask, val_mask, test_mask = get_split(nnodes,train_ratio=0.6, test_ratio=0.2)
    else:
